...
var providerName = "FMOD Script-Based Integration Example (Perforce)";

var p4Settings = {
	p4port: "perforce:1666",	// default value
	p4host: "",
	p4encoding: "none",
	p4user: "",	
    p4password: "",
...
function runP4( command, additionalArgs, stdInput ) {
    var base = "-z tag"; // output result of command in fstat format
    ...
    additionalArgs.map(function(arg) {
        processArgs.push(arg.toString());
    });
    
    var options = { timeout: 10000, args: processArgs, standardInput: stdInput }; // timeout is arbitary, depending on your server, you mind need longer timeout period
    var result = studio.system.start("p4", options);
    
    if (result.exitCode != 0) {
        console.error("Error in runP4!");
        return result;
    }
    ...
    while ((tokens = regex.exec(input)) != null) {
        ...
        if(dots === "...")
        {
            switch (key) {
                ...
                case "clientFile": result.clientFile = value; break;
                ...
                case "actionOwner":
                    break; // unused
                default: console.warn("unknown p4 key: ... " + key); break;
            }
        }
        else if(dots === "... ...")
        {
            ...
            else if(key == "otherOpen" || key == "otherLock" || key == "rev" || key == "client")
                ; // unused
            else
                console.warn("unknown p4 key: ... ... " + key);
        }
    }
...
function openedFiles( pathsToCheck ) {
    var result = runP4("opened", pathsToPerforceStylePaths(pathsToCheck));
    throwOnError(result);
...
function checkout( files, addIfNotInDepot ) {
    ...
    console.log(result.standardError);    
    console.log(addIfNotInDepot);    
...
function rename( fromFiles, toFiles ) {
    ...
    currentStats.map(function(stat) {
        if(stat.action == studio.sourceControl.action.Add) {
            var result = runP4("revert", [fromFiles[index]]);
            throwOnError(result);
            result = runP4("add", [toFiles[index]]);
            throwOnError(result);
        }
        else
        {
            var result = runP4("edit", [fromFiles[index]]);
            throwOnError(result);
            result = runP4("move", [fromFiles[index], toFiles[index]]);
            throwOnError(result);
        }
        index++;
    });
...
function revert( files, revertIfUnchanged ) {
    ...
    openedFiles(files).map(function(stat) {
        ...
        if(!revertIfUnchanged || stat.action != studio.sourceControl.action.Add) {
            filesToRevert.push(stat.depotFile);
        }
    });
    var result = runP4("revert", cmdArguments.concat(filesToRevert));
    throwOnError(result);
...
function print( fileToPrint, revisionToPrint, outputFilePath ) {
    ...
    cmdArguments.push("-q");
    cmdArguments.push("-o");    
    cmdArguments.push(outputFilePath);    
    if(revisionToPrint == 0) {
        cmdArguments.push(fileToPrint);    
    }
    else {
        cmdArguments.push(fileToPrint + "#" + revisionToPrint);    
    }

    var result = runP4("print", cmdArguments);
    throwOnError(result);
}
...
function pendingChanges( paths ) {
    ...
    if(depotPathOfOpenedFiles != undefined) {
        return fstat(depotPathOfOpenedFiles);
    }
}
function filterSyncError( result ) {
    // Perforce reckons if a file is already sync and we try to sync again, it's an error.
    // From Studio point of view, it is not a big deal so we are ignoring those errors
    var filteredErrors = [];
    result.standardError.trim().split("\r\n").map(function(block) {
        var regex = /^(.*) ((- file\(s\) up-to-date.)|(- no such file\(s\).))$/gmi;
        tokens = regex.exec(block);
        if(tokens == null)
...
function syncPreview( paths ) {
    var cmdArguments = [];
    cmdArguments.push("-n");
    var result = filterSyncError(runP4("sync", cmdArguments.concat(pathsToPerforceStylePaths(paths))));
    throwOnError(result);
...
function sync( fileList ) {
    var result = filterSyncError(runP4("sync", pathsToPerforceStylePaths(fileList)));
    throwOnError(result);
...
function resolve( acceptYours, acceptTheirs, acceptMerge ) {
    ...
    if(acceptYours.length) {
        var cmdArguments = [];
        cmdArguments.push("-ay");
        throwOnError(runP4("resolve", cmdArguments.concat(acceptYours)));
    ...
    if(acceptMerge.length) {
        var cmdArguments = [];
        cmdArguments.push("-ay");
        throwOnError(runP4("resolve", cmdArguments.concat(acceptMerge)));
...
function identifyLocalChanges( directoryList, fileList ) {
    ...
    dirList = directoryList.map(function(directory) { return directory.toString().replace(/^\/?|\/?$/g, ""); });
    ...
    mappedDirectories.forEach(function(directory) {
        ...
        filesResult.standardOutput.trim().split("\r\n\r\n").map(function(block) {
            var dict = createKeyValuePairs(block);
            if(dict.change != studio.sourceControl.action.Delete && dict.depotFile && dict.depotFile.length) {
    ...
    if(filesToReconcile.length) {
        console.log("Retrieving stats for files to be reconciled....");
        var fileStats = fstat(filesToReconcile);
        var fileStatsMap = [];
        var filesToAddOrCheckout = [];
        var filesToDelete = [];
        // put the filStats in a map for quicker lookup
        fileStats.forEach(function(stat) {
            fileStatsMap[stat.depotFile] = stat;
        });
        filesToReconcile.forEach(function(file) {
            if(file in fileStatsMap) {
                // perform a diff using perforce
                console.log("Performing diff for file " + file + "...");
                var diffResult = runP4("diff", ["-f", "-sl", fileStatsMap[file].clientFile + "#" + fileStatsMap[file].haveRevision]);
                ...
                if(diffStatus == "diff") { // the file is difference from server, add it to checkout list
                    filesToAddOrCheckout.push(file);
                }
                else if(diffStatus.indexOf("missing") != -1) { // file is not on local disk, mark it for delete
        ...
        if(filesToAddOrCheckout.length) {
            console.log("Checking out/Marking for add files....");
...
function checkin( changeDescription, filesToCheckin, checkinKeyFile ) {
    ...
    var result = runP4("change", ["-i"], "Change: new\nDescription: " + changeDescription + "\n");
    throwOnError(result);
    ...
    var reopenArguments = ["-c", changelistNumber];
    throwOnError(runP4("reopen", reopenArguments.concat(filesToCheckin)));
...
function findFiles( filter ) {
    ...
    return fstat(searchStrings(clientDescription, filter));
}

function availableUsers() {
    var result = runP4("users", []);
    var users = [];
    var index = 0;
    var currentIndex = -1;
    if(result.standardError.length == 0) {
        ...
        setProviderStatus(result.standardError);   
    }
...
function availableClients() {
    ...
    if(p4Settings.p4user && p4Settings.p4user.length > 0) {
        var result = runP4("clients -u " + p4Settings.p4user, []);
        if(result.standardError.length == 0) {
            result.standardOutput.trim().split("\r\n\r\n").map(function(block) {
                var dict = createKeyValuePairs(block);
                if(dict.client) {
            ...
            if(currentIndex == -1) {
                ...
                if(clients.length) {
                    ...
                    setProviderStatus("There is no available workspace for the user you have selected.");
                }
            }
            else {
                setProviderStatus("");
...
function refreshSettings(widget) {
    ...
    var hasConnectionChanges = false;
	var p4port = widget.findWidget("m_serverText").text();
	var p4host = widget.findWidget("m_hostText").text();
	var p4password = widget.findWidget("m_passwordText").text();
    var p4encoding = widget.findWidget("m_encodingCombo").currentUserData();
    var p4user = userCombo.currentText();

    hasConnectionChanges = widget.widgetId() == "m_layout" || p4port != p4Settings.p4port || p4host != p4Settings.p4host || p4password != p4Settings.p4password || p4encoding != p4Settings.p4encoding || p4user != p4Settings.p4user;
    if(!hasConnectionChanges)
        return;

    p4Settings.p4port = p4port;
    p4Settings.p4host = p4host;
    p4Settings.p4password = p4password;
    p4Settings.p4encoding = p4encoding;
...
function verifyWorkspaceSelection(widget) {
    ...
    if(projectPathToVerify && projectPathToVerify.length) {
        // check if the project path is within the workspace view
        var result = runP4("where", [projectPathToVerify]);
        setProviderStatus(result.standardError);
    }
...
function settingsUIDefinition(projectPath) {
...
{
	studio.menu.addMenuItem({ name: "Perforce Settings UI Test", execute: function() {
